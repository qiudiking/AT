...
use server\Sign\Sign;
use server\Verify\VerifyRule;
...
class BaseBootstrap {
	...
	public static function controllerVerify(\Yaf\Request_Abstract $request){
		...
		if ( class_exists( $controllerClass ) ) {
			...
			if ( ! $methodsDocInfo ) {
				$methodsDocInfo                       = \server\DocParse\ClassDocInfo::getMethodsInfo( $controllerClass );
	...
	public static function requestVerify( $valData ) {
		...
		$verifyRule           = \server\Verify\VerifyFactory::VerifyRule();
